...
FROM buyouts
         ...
         JOIN public.users on buyouts.user_id = users.id
WHERE user_id IS NOT NULL
  AND conf_time IS NOT NULL
...
FROM users AS u
         LEFT JOIN buyouts AS b ON u.id = b.user_id AND b.fact_time IS NULL
WHERE b.id IS NULL
  AND u.conf_time IS NOT NULL
